...
func (p *ResumeUploader) Mkblk(
	...
	headers := http.Header{}
	headers.Add("Content-Type", base.CONTENT_TYPE_OCTET)
...
func (p *ResumeUploader) resumableBput(
	...
	lzRetry:
		...
		if err == nil {
			...
			client.Warn("ResumableBlockput: invalid checksum, retry")
			err = ErrUnmatchedChecksum
		} else {
			if ei, ok := err.(*client.ErrorInfo); ok && ei.Code == InvalidCtx {
				ret.Ctx = "" // reset
				client.Warn("ResumableBlockput: invalid ctx, please retry")
				return
			}
			client.Warn("ResumableBlockput: bput failed -", err)
		}
		if tryTimes > 1 {
			tryTimes--
			client.Info("ResumableBlockput retrying ...")
			goto lzRetry
		}
		break
	}

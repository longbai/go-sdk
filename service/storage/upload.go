...
import (
	...
	log "github.com/longbai/go-sdk/internal"
...
func (r *defaultChunkRetryer) Retry(ck *Chunk) {
	...
	if i >= r.maxCount {
		ck.Err = base.NewError(ErrMaxUpRetry, "exceed max retry times")
	}
}
...
func (u *uploader) upload(ret interface{}, key string, hasKey bool, extra *RputExtra) error {
	...
	if nerr != nil && nerr != io.EOF {
		...
		//  等待所有的worker结束
		u.wg.Wait()
		return base.NewError(ErrNextReader, nerr.Error())
	}
	u.wg.Wait()
	if u.totalSize == -1 {
